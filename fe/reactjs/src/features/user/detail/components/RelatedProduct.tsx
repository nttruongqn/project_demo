...
export function RelatedProduct({ relatedProducts }: IRelatedProductProps) {
    const navigate = useNavigate();

    const handleGetProduct = (slug: string) => {
        navigate(`/dien-thoai/${slug}`);
        window.scrollTo(0, 0);
    }
    ...
    return (
        ...
        <section className="my-2 max-md:px-2 md:bg-white md:px-4 md:pb-4 md:rounded-md">
            <h2 className="text-red-700 font-bold text-lg flex max-md:justify-start my-2 md:text-2xl md:hidden">Sản
                phẩm liên quan</h2>
            <div className="flex justify-between items-center max-md:hidden">
                <h2 className="text-red-700 font-bold text-lg flex max-md:justify-start py-4 md:text-2xl">Sản phẩm
                    tương tự</h2>
            </div>
            <div
                ...
                {relatedProducts?.map((item, index) => (
                    <div
                        className="max-md:min-w-[180px] max-md:h-[300px] p-2 bg-white md:col-span-1 md:border md:rounded cursor-pointer" key={index} onClick={() => handleGetProduct(item.slug as string)}>
                        <div className="max-md:h-auto">
                        <motion.img whileHover={{ scale: 0.9 }} src={item.imageUrl} alt="" />                        </div>
